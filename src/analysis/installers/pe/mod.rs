...
pub use vs_version_info::VSVersionInfo;
...
impl PE {
        ...
        R: Read + Seek,
    {
        ...
        let version_directory_table = version_entry
            .data(&mut resource_directory)?
            .table()
        ...
        let version_data_entry_offset = version_directory.file_offset(resource_directory_offset);
        reader.seek(SeekFrom::Start(version_data_entry_offset.into()))?;

        let version_data_entry = reader.read_t::<ImageResourceDataEntry>()?;
        let version_offset = self
            .section_table
            .to_file_offset(version_data_entry.offset_to_data())?;

        reader.seek(SeekFrom::Start(version_offset.into()))?;
        let mut version_reader = reader.take(version_data_entry.size().into());

        let mut version_info = Vec::with_capacity(version_data_entry.size() as usize);
        version_reader.read_to_end(&mut version_info)?;
