...
impl Squirrel {
    pub fn new<R: Read + Seek>(mut reader: R, pe: &PE) -> Result<Self, SquirrelError> {
        ...
        let (mut nupkg, is_velopack) = match ZipArchive::new(Cursor::new(&resource_data)) {
            // Squirrel
            Ok(mut zip) => {
                // A surprising number of custom installers also store zips in the first resource
                zip.file_names().for_each(|file| debug!(file));
        ...
        nupkg.file_names().for_each(|file| debug!(%file));
        ...
        let entrypoint = nupkg.file_names().find_map(|name| {
            let stem = name.rsplit('/').next()?.strip_suffix(".exe")?;
            (nuspec
                .metadata
                .main_exe
                .as_deref()
