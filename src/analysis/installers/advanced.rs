...
use winget_types::installer::{
    ExpectedReturnCodes, Installer, InstallerReturnCode, InstallerSwitches, InstallerType,
    ReturnResponse,
};
use yara_x::mods::PE;
...
impl AdvancedInstaller {
    pub fn new<R: Read + Seek>(mut reader: R, pe: &PE) -> Result<Self, AdvancedInstallerError> {
        ...
        // 10-byte magic before EOF or cert table, padded with trailing nulls to 8-byte boundary
        let search_start = end_offset.saturating_sub(17);
        reader.seek(SeekFrom::Start(search_start))?;
        let mut buf = vec![0u8; (end_offset - search_start) as usize];
        reader.read_exact(&mut buf)?;
        let magic_pos = buf
            .windows(10)
            .rposition(|w| w == b"ADVINSTSFX")
        ...
        let seven_z_data = files
            .iter()
            .rev()
            .find(|f| f.name.to_ascii_lowercase().ends_with(".7z"))
            .and_then(|f| f.read(&mut reader).ok())
            .ok_or(AdvancedInstallerError::NotAdvancedInstallerFile)?;

        let mut installers = Vec::new();
        ArchiveReader::new(Cursor::new(&seven_z_data), Password::empty())?.for_each_entries(
            |_entry, reader| {
                let mut buf = Vec::new();
        ...
        self.installers
            ...
            .map(|msi| {
                ...
                installer.switches = InstallerSwitches::builder()
                    .silent("/exenoui /quiet /norestart".parse().unwrap())
                    .silent_with_progress("/exenoui /passive /norestart".parse().unwrap())
                    .install_location("APPDIR=\"<INSTALLPATH>\"".parse().unwrap())
                    .log("/log \"<LOGPATH>\"".parse().unwrap())
                    .maybe_custom(installer.switches.custom().cloned())
                    .build();
...
pub struct File {
    ...
    pub size: u32,
    pub offset: u32,
    pub encoding_flag: u32,
}

impl File {
    pub fn read<R: Read + Seek>(&self, reader: &mut R) -> std::io::Result<Vec<u8>> {
