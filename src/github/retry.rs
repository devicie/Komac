...
impl GitHub {
    ...
    where
        ...
        Vars: Serialize,
    {
        // TEMPORARY TEST EXIT 1: remove this flag after validating workflow retries.
        let mut retried_transient_graphql_error = false;
        ...
        for retry in 0..=MAX_GITHUB_REQUEST_RETRIES {
            ...
            if retry == MAX_GITHUB_REQUEST_RETRIES || !is_retryable_graphql_response(&response) {
                // TEMPORARY TEST EXIT 1: remove this block after validating workflow retries.
                if retried_transient_graphql_error {
                    return Err(GitHubError::GraphQL(color_eyre::eyre::eyre!(
                        "temporary forced failure after GitHub GraphQL retry for workflow retry validation"
                    )));
                }
            ...
            // TEMPORARY TEST EXIT 1: remove this assignment after validating workflow retries.
            retried_transient_graphql_error = true;
